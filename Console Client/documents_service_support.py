...
class PUTRequest(urllib2.Request):
	def get_method(self): return "PUT"

class DELETERequest(urllib2.Request):
	def get_method(self): return "DELETE"

def GetUserAgent():
	product_tokens = []
	product_tokens.append('documents_service_py/%s' % "1.0")
	product_tokens.append(GetPlatformToken())
...
class AbstractRpcServer(object):
  ...
  def _CreateRequest(self, url, data=None, method=None):
  ...
  def Send(self, request_path, body=None, method=None,
    ...
    Args:
      ...
      body: The body of the request, or None to send an empty request.
      method: HTTP Method.
    ...
    try:
      ...
      while True:
        ...
        url = "%s://%s%s?%s" % (self.scheme, self.host, request_path,
                                urllib.urlencode(args))
        req = self._CreateRequest(url=url, data=body, method=method)
...
class HttpRpcServer(AbstractRpcServer):
  ...
  DEFAULT_COOKIE_FILE_PATH = "~/.documents_service_cookies"
